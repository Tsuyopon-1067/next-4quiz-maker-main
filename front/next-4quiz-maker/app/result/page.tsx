...
export default function Result() {
    const [resultJsonData, setResultJsonData] = useState<ResultJson[] | null>(
        null
    );
    ...
    return (
        <div className={styles.main}>
            <div className={styles.title_bar}>解答</div>
                ...
                {resultJsonData.map((elem: ResultJson, index: number) => (
                    <div key={index}>
                        <ResultArea
                            num={index + 1}
                            selected={
                                quizJsonData[index].options[elem.Selected]
                            }
                            answer={quizJsonData[index].options[elem.Answer]}
                            questionUrl={
                                "http://localhost:50000/HTML/question/" +
                                quizJsonData[index].file
                            }
                            answerUrl={
                                "http://localhost:50000/HTML/answer/" +
                                quizJsonData[index].file
                            }
                        />
                    </div>
