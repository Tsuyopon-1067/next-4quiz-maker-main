...
export default function Question() {
    ...
    const [jsonData, setJsonData] = useState<QuizJson[]>([]);
    ...
    const sendOption = (selectedOption: number) => {
        fetch(
            "http://localhost:50000/SEND_SELECT/" +
                nowPage +
                "/" +
                selectedOption,
            { method: "GET" }
    ...
    useEffect(() => {
        ...
        if (jsonData != null) {
            setQuestionUrl(
                "http://localhost:50000/HTML/" +
                    "question/" +
                    jsonData[nowPage - 1].file
            );
        }
    }, [jsonData, nowPage]);
    ...
    return (
        <div className={styles.main}>
            <div className={styles.title_bar}>第{nowPage}問</div>
            <HtmlArea url={questionUrl} />
            <div className={styles.bottom_container}>
                <div className={styles.radio_button_area}>
                    <RadioButtonGroup
                        optionList={jsonData[nowPage - 1].options}
                        onChange={sendOption}
...
const ButtomButtoms: FC<ButtomButtomsProps> = (props) => {
    ...
    if (nowPage < lastPage) {
        buttons = (
            <>
                <button className={styles.arrow_button} onClick={prevPage}>
                    ⇦
                </button>
                <button className={styles.arrow_button} onClick={nextPage}>
                    ⇨
                </button>
            </>
        );
        ...
        buttons = (
            <>
                <button className={styles.arrow_button} onClick={clickEnd}>
                    終了
                </button>
                <button className={styles.arrow_button} onClick={prevPage}>
                    ⇦
                ...
                <button
                    className={`${styles.arrow_button} ${styles.hide_button}`}
                    onClick={nextPage}
                >
                    ⇨
                </button>
    ...
    return <div className={styles.arrow_button_area}>{buttons}</div>;
};
